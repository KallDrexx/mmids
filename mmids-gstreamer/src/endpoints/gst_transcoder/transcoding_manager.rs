...
pub struct TranscoderParams {
    ...
    inbound_media: UnboundedReceiver<MediaNotificationContent>,
    outbound_media: UnboundedSender<MediaNotificationContent>,
    pipeline: Pipeline,
}
...
enum TranscoderFutureResult {
    ...
    RequestReceived(TranscodeManagerRequest, UnboundedReceiver<TranscodeManagerRequest>),
    MediaReceived(MediaNotificationContent, UnboundedReceiver<MediaNotificationContent>),
    GstBusClosed,
    GstEosReceived,
...
impl TranscodeManager {
        ...
        while let Some(result) = self.futures.next().await {
            match result {
                ...
                TranscoderFutureResult::MediaReceived(media, receiver) => {
                    self.futures
                        .push(notify_on_inbound_media(receiver).boxed());
                ...
                TranscoderFutureResult::RequestReceived(request, receiver) => {
                    self.futures
                        .push(wait_for_request(receiver).boxed());
    ...
    fn handle_media(&self, media: MediaNotificationContent) {
...
async fn notify_bus_message(mut bus: BusStream) -> TranscoderFutureResult {
    while let Some(message) = bus.next().await {
        match message.view() {
            ...
            MessageView::Error(error) => return TranscoderFutureResult::GstErrorReceived(GstError {
                source_name: error.src()
