...
async fn response_examples(req: Request<Body>) -> Result<Response<Body>> {
    match (req.method(), req.uri().path()) {
        ...
        (&Method::POST, path) => {
            ...
            let name = match String::from_utf8(whole_body.to_vec()) {
                Ok(name) => name,
                Err(error) => {
                    return Ok(Response::builder()
                        .status(StatusCode::BAD_REQUEST)
                        .body(format!("Error reading body: {:?}", error).into())
                        .unwrap());
                }
            };

            println!("Request came in for {}/'{}'", sub_directory, name);

            let path = format!("{}/{}/{}.mmids", DIRECTORY, sub_directory, name);
