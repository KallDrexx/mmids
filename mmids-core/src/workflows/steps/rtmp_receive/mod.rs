...
use crate::StreamId;
use crate::endpoints::rtmp_server::{RtmpEndpointRequest, StreamKeyRegistration, RtmpEndpointPublisherMessage};
...
enum RtmpReceiveFutureResult {
    RtmpEndpointResponseReceived(RtmpEndpointPublisherMessage, UnboundedReceiver<RtmpEndpointPublisherMessage>),
    RtmpEndpointGone,
}
...
impl WorkflowStep for RtmpReceiverStep {
    fn init<'a>(&mut self) -> Vec<BoxFuture<'a, Box<dyn StepFutureResult>>> {
        ...
        let _ = self.rtmp_endpoint_sender.send(RtmpEndpointRequest::ListenForPublishers {
